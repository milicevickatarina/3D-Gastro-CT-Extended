...
def show_hist(main_dir, x_left, x_right, y_top, y_bottom, z_top, z_bottom):
    try:
        ...
        kidney_box_sitk = sitk.GetImageFromArray(kidney_box)
        kidney_box_sitk.SetSpacing(img.GetSpacing())
        sitk.WriteImage(kidney_box_sitk,'kidney_box32.mha')
...
def right_kidney(main_dir, Gmin, Gmax, x_left, x_right, y_top, y_bottom, z_top, z_bottom):
    try:
        img = sitk.ReadImage(main_dir + "/data/vein_phase_preprocessed.mha")
        img_array = sitk.GetArrayFromImage(img)
        ...
        cleaned_thresh_img = sitk.BinaryOpeningByReconstruction(kidney_sitk, [3, 3, 3])
        kidney_sitk = sitk.BinaryClosingByReconstruction(cleaned_thresh_img, [3, 3, 3])
